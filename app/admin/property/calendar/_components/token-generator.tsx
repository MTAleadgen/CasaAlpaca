...
interface CalendarTokenGeneratorProps {
  propertyId?: string
}

export function CalendarTokenGenerator({
  propertyId
}: CalendarTokenGeneratorProps) {
  ...
  const calendarUrl = propertyId
    ? `${baseUrl}/api/calendar/${propertyId}/${calendarToken}`
    : `${baseUrl}/api/calendar/${calendarToken}`
  ...
  const handleGenerateToken = async () => {
    ...
    try {
      // In a real implementation, we would call the server action with propertyId
      // const result = await generateCalendarTokenAction(propertyId)
      // if (result.isSuccess) {
