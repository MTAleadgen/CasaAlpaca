...
import {
  ...
  getPropertyPhotosAction
} from "@/actions/property-actions"
import { SelectPropertyPhoto } from "@/db/schema"
...
export function PhotoGallery({ propertyId }: PhotoGalleryProps) {
  ...
  const [photos, setPhotos] = useState<SelectPropertyPhoto[]>([])
  ...
  useEffect(() => {
    const loadPhotos = async () => {
      setIsLoading(true)
      try {
        const result = await getPropertyPhotosAction(propertyId)
        if (result.isSuccess) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Loading photos...</CardDescription>
        </CardHeader>
      </Card>
    )
  }
          ...
          {photos.map(photo => (
            ...
            >
              <div className="relative aspect-square">
                <Image
                  src={photo.url}
                  alt={photo.alt || `Property photo ${photo.id}`}
                  fill
